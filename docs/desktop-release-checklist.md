...
1. **Install, and get past the OS guard on first launch.**
   - macOS: mount the `.dmg`, copy Pikchard to `/Applications`, then
     double-click it. Gatekeeper blocks it. Click
     **System Settings › Privacy & Security › Open Anyway**, and it launches.
     This is also the only check that the release job's ad-hoc signature
     worked. An unsigned universal app does not run on Apple silicon at all.
...
not ship. If an item was not run, the artifact ships behind the README's
